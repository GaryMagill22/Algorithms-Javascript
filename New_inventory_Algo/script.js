...
function updateInventory(newInv, currInv) {
    for (let i = 0; i < newInv.length; i++) {
        ...
        for (let j = 0; j < currInv.length; j++) {
            if (currInv[j].name === newItem.name) {
                currInv[j].quantity += newItem.quantity;
                found = true;
                break;
            }
        }
    ...
    currInv.sort((a, b) => a.name.localeCompare(b.name));
    return currInv;
}

const updatedInv2 = updateInventory(newInv2, currInv2)
